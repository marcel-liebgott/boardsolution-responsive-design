...
final class BS_Functions extends FWS_Object
{
	...
	/**
	 * Builds a custom text for an "order-column"
	 *
	 * @param string $title the title of the column
	 * @param string $order_value the value of the order-parameter
	 * @param string $def_ascdesc the default value for BS_URL_AD (ASC or DESC)
	 * @param string $order the current value of BS_URL_ORDER
	 * @param BS_URL $url the current URL
	 * @param string $asc the css class for the asc image
	 * @param string $desc the css class for the desc image
	 * @return string the column-content
	 */
	public function get_custom_order_column($title,$order_value,$def_ascdesc,$order,$url,$asc,$desc)
	{
		if(!($url instanceof BS_URL))
			FWS_Helper::def_error('instance','url','BS_URL',$url);
		...
		if($order == $order_value)
		{
			$asc_img = '<i class="'.$asc.'"></i>';
			$desc_img = '<i class="'.$desc.'"></i>';
			$result = $title.' <a href="'.$url->set(BS_URL_AD,'ASC')->to_url().'">'.$asc_img.'</a> <a href="'.$url->set(BS_URL_AD,'DESC')->to_url().'">' . $desc_img;'</a>';
		}
		else
			$result = '<a href="'.$url->set(BS_URL_AD,$def_ascdesc)->to_url().'">'.$title.'</a>';
	...
	public function get_custom_link_rating($total,$votes)
	{
		$max = 6;
		...
		if($votes == 0)
			...
			$result = @round($total / $votes,2);
		
		$per = @round(100 / ($max / $result),2);

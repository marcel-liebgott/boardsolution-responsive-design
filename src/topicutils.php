...
final class BS_TopicUtils extends FWS_UtilBase
{
	...
	/**
	 * generates the symbols for topics as array
	 *
	 * mliebgott theme
	 *
	 * @param BS_HTML_Formular $form the formular
	 ...
	 * @return string the symbols
	 */
	public static function get_symbols_array($form,$select = 0)
	{
		$locale = FWS_Props::get()->locale();
		$user = FWS_Props::get()->user();
		...
		{			
			if($i == 0)
				$symbols[$i] = $locale->lang('noicon');
			else
			{
				$img = $user->get_theme_item_path('images/thread_type/symbol_'.$i.'.gif');
				$symbols[$i] = '<img src="'.$img.'" alt="" />';
			}
		}
	...
	public static function get_action_combobox($location = 'topics',$is_closed = false)
	{
		...
		// mliebgott theme modified
		$var = '<select id="topic_action" name="topic_action" class="form-control"';

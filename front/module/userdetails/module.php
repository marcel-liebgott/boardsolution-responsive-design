...
final class BS_Front_Module_userdetails extends BS_Front_Module
{
	...
	public function run()
	{
		...
		if($cfg['enable_avatars'] == 1)
		{
			...
			$tpl->add_variables(array(
				...
				'avatar_url' => BS_UserUtils::get_profile_avatar_url(
					$user_data['avatar'],$user_data['id']
				)
			));
		}
		...
		$tpl->add_variables(array(
			...
			'options' => $options,
			'options_array' => $this->_get_option_array(),
	 ...
	 * mliebgott theme
	 */
	private function _get_option_array(){
		$locale = FWS_Props::get()->locale();
		$input = FWS_Props::get()->input();
		$cfg = FWS_Props::get()->cfg();
		$sessions = FWS_Props::get()->sessions();
		...
		$location = $sessions->get_user_location($user_data['id']);
		if($user_data['ghost_mode'] == 0 || $cfg['allow_ghost_mode'] == 0 || $user->is_admin())
		{
			$loc = '';
			if($auth->has_global_permission('view_online_locations'))
			{
			...
			if($location != ''){
				$online = true;
				$status_desc = $locale->lang('online');
			}else{
				$online = false;
				$status_desc = $locale->lang('offline');
			}

			$options['user_status'] = array(
				'url'		=> BS_URL::build_mod_url('user_locations'),
				'loc'		=> $loc,
				'online'	=> $online,
				'locale' 	=> $status_desc,
				'title'		=> $status_desc
		...
		if(($cfg['display_denied_options'] || $user->is_loggedin()) && $cfg['enable_pms'] == 1 &&
			$user->get_profile_val('allow_pms') == 1 && $user_data['allow_pms'] == 1 && $user_data['banned'] == 0)
		{
			$url = BS_URL::get_sub_url('userprofile','pmcompose');
			$url->set(BS_URL_ID,$user_data['id']);
			
			$options['send_pm'] = array(
				'url' 		=> $url->to_url(),
				'title' 	=> sprintf($locale->lang('send_pm_to_user'),$user_data['user_name']),
				'locale' 	=> $locale->lang('pm_short')
			);
		}
		
		if(($cfg['display_denied_options'] || $auth->has_global_permission('send_mails')) &&
			 $user_data['allow_board_emails'] == 1 && $cfg['enable_emails'] == 1 && $user_data['user_email'] != '')
		{
			$url = BS_URL::get_mod_url('new_mail');
			$url->set(BS_URL_ID,$user_data['id']);
			
			$options['email'] = array(
				'title' 	=> sprintf($locale->lang('send_mail_to_user'),$user_data['user_name']),
				'url' 		=> $url->to_url(),
				'locale'	=> $locale->lang('email'),
				'title'		=> $locale->lang('email')
			);
		}

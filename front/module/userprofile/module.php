...
final class BS_Front_Module_userprofile extends BS_Front_SubModuleContainer
{
	...
	public function run()
	{
		...
		$tpl->add_variables(array(
			...
			// mliebgott theme
			'inbox_num_int' => $inbox_num,
			'inbox_num_acp' => $cfg['pm_max_inbox'],
			'inbox_num_pro' => $cfg['pm_max_inbox'] > 0 ? ($inbox_num / $cfg['pm_max_inbox']) * 100 : false,
			'outbox_num_int' => $outbox_num,
			'outbox_num_acp' => $cfg['pm_max_outbox'],
			'outbox_num_pro' => $cfg['pm_max_outbox'] > 0 ? ($outbox_num / $cfg['pm_max_outbox']) * 100 : false
		));
	}

	...
	public function run()
	{
		...
		if($num > 0)
		{
			...
			if($qry_num)
			{
				...
				$vote_options_rating = array(
					$locale->lang('please_choose'),
					'1 &ndash; '.$locale->lang('verybad'),
					'2 &ndash; '.$locale->lang('bad'),
					'3 &ndash; '.$locale->lang('acceptable'),
					'4 &ndash; '.$locale->lang('middle'),
					'5 &ndash; '.$locale->lang('good'),
					'6 &ndash; '.$locale->lang('verygood')
				);
				...
				foreach($linklist as $i => $data)
				{
					...
					$links[] = array(
						...
						// mliebgott theme
						'vote_options_rating' => $vote_options_rating,
						'vote_rating_result' => $functions->get_custom_link_rating($data['vote_points'],$data['votes'])
					);

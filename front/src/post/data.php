...
final class BS_Front_Post_Data extends FWS_Object
{
	...
	/**
	 * mliebgott theme
	 * @return array the post-buttons to display
	 */
	public function get_post_buttons_array()
	{
		$auth = FWS_Props::get()->auth();
		$forums = FWS_Props::get()->forums();
		$input = FWS_Props::get()->input();
		...
		if($cache['action'] == 'posts' && ($cache['is_admin'] || !$cache['fclosed']))
		{
							...
							$cache['posts_order'] == 'DESC'))
			{
						...
						$auth->has_current_forum_perm(BS_MODE_DELETE_TOPICS,$this->_data['post_user']))
				{
					...
					$btns['delete_topic'] = array(
						'url' => $murl->to_url(),
						'locale' => $locale->lang('delete_topic')
					);
				}
			}
			else
			{
								...
								($cache['is_admin'] || $cache['topic']['thread_closed'] == 0)))
				{
					...
					$btns['delete_post'] = array(
						'url' => $murl->to_url(),
						'locale' => $locale->lang('deletepost')
					);
				}
			}
					  ...
					  ($cache['is_admin'] || $cache['topic']['thread_closed'] == 0)))
			{
				...
				$btns['edit_post'] = array(
					'url' => $murl->to_url(),
					'locale' => $locale->lang('editpost')
				);
			}
					...
					($cache['is_admin'] || $cache['topic']['thread_closed'] == 0))
			{
				...
				$btns['quote_this_post'] = array(
					'url' => $murl->to_url(),
					'locale' => $locale->lang('quotethispost'),
					'onclick' => 'toggleQuote(\''.$cache['ajax_qurl'].'\','.$this->_data['bid'].'); return false',
					'id' => 'quote_link_'.$this->_data['bid']
				);
			}
		}
	...
	public function get_profile_buttons()
	{
		...
		if($this->_data['banned'] == 0 && $cfg['enable_pms'] == 1 && $this->_data['post_user'] != 0 &&
			 $user->get_profile_val('allow_pms') == 1 && $this->_data['allow_pms'] == 1 &&
			 ($cfg['display_denied_options'] || $cache['is_loggedin']))
		{
			$murl = $cache['pm_url'];
			$murl->set(BS_URL_ID,$this->_data['post_user']);
			$btns .= '<a class="bs_button" style="float: left;" title="';
